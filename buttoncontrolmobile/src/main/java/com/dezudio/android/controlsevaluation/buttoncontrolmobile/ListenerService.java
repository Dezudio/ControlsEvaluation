...
public class ListenerService extends Service {
    private final static String TAG = ListenerService.class.getSimpleName();
...
/*                for( BluetoothGattService i : mBluetoothGatt.getServices()) {
                    Log.d(TAG, "Service:" + i.getUuid().toString());
                    for(BluetoothGattCharacteristic j : i.getCharacteristics()) {
                        Log.d(TAG, "Characteristic: " + j.getUuid().toString());
                    }
                }*/
                BluetoothGattService foo =
                        gatt.getService(UUID.fromString("c97433f0-be8f-4dc8-b6f0-5343e6100eb4"));
                if( foo != null) {
                    BluetoothGattCharacteristic bar = foo.getCharacteristic(UUID.fromString("c97433f1-be8f-4dc8-b6f0-5343e6100eb4"));
                    final int charaProp = bar.getProperties();
                    if ((charaProp | BluetoothGattCharacteristic.PROPERTY_NOTIFY) > 0) {
                        Log.d(TAG, "Enable Notify");
                       /* readCharacteristic(bar);
                        BluetoothGattDescriptor descriptor = bar.getDescriptor(
                                UUID.fromString(ButtonGattAttributes.CLIENT_CHARACTERISTIC_CONFIG));
                        setCharacteristicNotification(bar, true);
    ...
    private void broadcastUpdate(final String action,
        ...
        i.putExtra(TIMESTAMP_KEY, timestamp);
        sendBroadcast(i);
